...
8.3 For the offset, we first need to figure out the number of blocks we have to skip, if any, by dividing offset by BLOCK_SIZE and then we create a current offset by doing offset % BLOCK_SIZE. We will then update the current offset by how much we write. 
---
